...
if not st.session_state.logged_in:
    ...
    st.markdown(f"""
        <div class="update-log">
            <b>🚀 업데이트 로그 ({now_d})</b><br>
            • <b>입력창 콤마 적용</b>: 모든 숫자 입력 필드에 천 단위 콤마 표기 수정<br>
            • <b>정산 기간 자동 표기</b>: 리포트 상단에 "X월 X일 ~ X월 X일" 기간 정보 추가<br>
            • <b>가독성 강화</b>: 예상 수령액 및 합계 금액의 폰트 크기 확대
        </div>
    """, unsafe_allow_html=True)
    st.stop()
...
with st.sidebar:
    ...
    if user_name == "태완":
        ...
        if st.button(f"💿 {target_staff} 설정 저장", use_container_width=True):
            ...
            log_msg = f"{get_now_kst().strftime('%m-%d %H:%M:%S')} | {target_staff} 설정 변경됨"
            st.session_state.admin_log.insert(0, log_msg)
            st.success("저장되었습니다."); st.rerun()
            
        if st.session_state.admin_log:
            st.markdown("**📝 최근 변경 이력**")
            log_display = "<br>".join(st.session_state.admin_log[:5])
...
if not existing_row.empty:
    ...
    st.markdown(f'<div class="save-log">📝 {str_date} {save_time} 저장됨</div>', unsafe_allow_html=True)
...
# 기간 텍스트 표시 (예: 1월 13일 ~ 2월 12일)
period_str = f"📅 {start_dt.month}월 {start_dt.day}일 ~ {end_dt.month}월 {end_dt.day}일"
st.markdown(f'<div class="period-text">{period_str}</div>', unsafe_allow_html=True)
