...
with st.sidebar:
    st.header("⚙️ 시스템 설정")
    if st.session_state.user_name == "태완":
        st.subheader("🛠️ 관리자 메뉴")
        target_staff = st.selectbox("수정할 직원 선택", STAFF_LIST)
        
        st.write("---")
        st.write("**📦 품목 단가 및 이름 설정**")
        item_names = []
        item_prices = []
        orig_names = ['일반필름', '풀필름', '젤리', '케이블', '어댑터', '추가1', '추가2']
        orig_prices = [9000, 18000, 9000, 15000, 23000, 0, 0]
        
        for i in range(7):
            c1, c2 = st.columns([1.5, 1])
            n = c1.text_input(f"품목{i+1} 이름", value=orig_names[i], key=f"n_{i}")
            p = c2.number_input(f"단가{i+1}", value=orig_prices[i], step=1000, format="%d", key=f"p_{i}")
            item_names.append(n)
            item_prices.append(p)
            
        st.write("---")
        st.write("**💰 정산 기본값 설정**")
        base_sal = st.number_input("기본급 (콤마 자동)", value=3500000, step=10000, format="%d")
        st.caption(f"현재: {base_sal:,}원")
        start_d = st.slider("정산 시작일", 1, 31, 13)
        insure = st.number_input("보험료 설정", value=104760, format="%d")
        st.caption(f"현재: {insure:,}원")
...
if is_edit:
    ...
    st.markdown(f'<div class="status-box" style="background-color: {status_color};">📌 {str_date} 기록 존재 ({reg_time} 저장)</div>', unsafe_allow_html=True)
else:
    st.markdown(f'<div class="status-box" style="background-color: #fafafa;">📝 실적을 입력하세요.</div>', unsafe_allow_html=True)
...
if st.session_state.incen_history:
    ...
    st.markdown(f'<div class="incen-log">📋 추가 로그: {log_text}</div>', unsafe_allow_html=True)
