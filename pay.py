...
st.markdown("""
    ...
    /* 버튼 3개 나란히 */
    .stButton button { width: 100%; padding: 8px 2px; font-size: 13px; }
    /* 수량 입력창 가로 2개 고정 */
    ...
    .incen-log { font-size: 11px; color: #666; margin-bottom: 10px; padding: 8px; background: #fdfdfd; border-radius: 5px; border-left: 3px solid #ddd; }
    </style>
...
if not st.session_state.logged_in:
    ...
    if st.button("입장하기"):
        if user_id == "태완" and admin_pw != "102030": st.error("비밀번호 불일치")
...
with st.sidebar:
    ...
    if user_name == "태완":
        ...
        st.write("**📦 품목 설정 (이름/단가)**")
        new_names = []
        new_prices = []
        for i in range(7):
            c1, c2 = st.columns([1.5, 1])
            n = c1.text_input(f"이름{i+1}", value=st.session_state.config["item_names"][i], key=f"sn_{i}", label_visibility="collapsed")
            p = c2.number_input(f"단가{i+1}", value=st.session_state.config["item_prices"][i], step=1000, key=f"sp_{i}", label_visibility="collapsed")
            new_names.append(n); new_prices.append(p)
        
        st.write("**💰 정산 설정**")
        base = st.number_input("기본급", value=st.session_state.config["base_salary"], step=10000); st.caption(f"{base:,}원")
        s_day = st.slider("시작일", 1, 31, st.session_state.config["start_day"])
        ins = st.number_input("보험료", value=st.session_state.config["insurance"]); st.caption(f"{ins:,}원")
        
        if st.button("💿 설정 저장 및 적용", use_container_width=True):
            ...
            st.session_state.admin_log.insert(0, f"[{get_now_kst().strftime('%H:%M')}] {target_staff} 설정 변경됨")
            st.success("저장 완료!")
        
        if st.session_state.admin_log:
            st.info("📜 변경 로그\n" + "\n".join(st.session_state.admin_log[:3]))
...
# 상태 표시
if is_edit:
    st.markdown(f'<div class="status-box" style="background-color: #e3f2fd;">📌 {str_date} 기록 ({existing_row.iloc[0]["입력시간"]} 저장)</div>', unsafe_allow_html=True)
else:
    st.markdown('<div class="status-box" style="background-color: #fafafa;">📝 실적을 입력하세요.</div>', unsafe_allow_html=True)
...
if st.session_state.incen_history:
    st.markdown(f'<div class="incen-log">📋 추가 내역: {" + ".join([f"{a:,}" for a in st.session_state.incen_history])}</div>', unsafe_allow_html=True)
...
if not df_all.empty:
    ...
    if not p_df.empty:
        ...
        st.info(f"📅 기간: {start_dt.strftime('%m/%d')} ~ {end_dt.strftime('%m/%d')}")
