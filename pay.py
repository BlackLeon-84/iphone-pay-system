...
with st.sidebar:
    ...
    with st.form("settings_form"):
        ...
        st.divider()
        confirm_pw = st.text_input("수정 승인 비밀번호", type="password")
...
st.markdown('<p class="version-text">v1.1.8-stable</p>', unsafe_allow_html=True)
...
if not period_df.empty:
    ...
    st.markdown(f"""<div style="background-color:#f0f2f6; padding:15px; border-radius:10px; border-left:5px solid #ff4b4b; margin-bottom:15px;">
        ...
        <p style="margin:0; font-size:22px; font-weight:bold; color:#ff4b4b;">🏦 실수령: {final_pay:,}원</p></div>""", unsafe_allow_html=True)
    
    html_code = f"""<table class="report-table"><tr><th style="width:35px;">날짜</th><th>인센</th><th>{item_names[0][:2]}</th><th>{item_names[1][:2]}</th><th>{item_names[2][:2]}</th><th>{item_names[3][:2]}</th><th>{item_names[4][:2]}</th><th>합계</th></tr>"""
