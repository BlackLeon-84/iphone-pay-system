...
def init_db():
    ...
    # [신규] 직원별 급여 환경 설정 테이블 (기본급, 정산 시작일)
    ...
    if c.fetchone()[0] == 0:
        default_items = [('item1', '일반필름', 9000), ('item2', '풀필름', 18000), 
                         ('item3', '젤리', 9000), ('item4', '케이블', 15000), ('item5', '어댑터', 23000)]
        c.executemany("INSERT INTO settings_v2 VALUES (?, ?, ?)", default_items)
...
def load_staff_config(name):
    ...
    conn.close()
    if df.empty:
        return {"base_salary": 3500000, "start_day": 13} # 기본값
...
with st.sidebar:
    ...
    if user_name == "태완":
        # 1. 항목 및 단가 관리
        st.subheader("📦 품목 단가 관리")
        df_items = load_settings()
        new_items = []
        with st.form("items_form"):
            for i, row in df_items.iterrows():
                n_name = st.text_input(f"품목{i+1}", value=row['display_name'], key=f"it_n_{row['id']}")
                n_price = st.number_input(f"가격", value=int(row['price']), step=1000, key=f"it_p_{row['id']}")
                new_items.append((n_name, n_price, row['id']))
            if st.form_submit_button("품목 저장"):
                conn = get_connection()
                conn.cursor().executemany("UPDATE settings_v2 SET display_name=?, price=? WHERE id=?", new_items)
                conn.commit()
                conn.close()
                st.rerun()

        # 2. 직원별 급여/날짜 설정
        st.subheader("💰 직원별 급여 설정")
        target_staff = st.selectbox("설정할 직원", STAFF_LIST)
        config = load_staff_config(target_staff)
        with st.form("staff_config_form"):
            new_base = st.number_input("기본급", value=int(config['base_salary']), step=10000)
            new_start_day = st.number_input("정산 시작일 (매월)", value=int(config['start_day']), min_value=1, max_value=28)
            if st.form_submit_button(f"{target_staff} 설정 저장"):
                conn = get_connection()
                conn.cursor().execute("INSERT OR REPLACE INTO staff_configs VALUES (?, ?, ?)", 
                                      (target_staff, new_base, new_start_day))
                conn.commit()
                conn.close()
                st.success(f"{target_staff}님 설정 완료")
                st.rerun()
    else:
        st.info("설정 권한이 없습니다.")
...
st.markdown('<p class="version-text">v1.1.9-stable</p>', unsafe_allow_html=True)
...
st.divider()
st.subheader("📊 정산 리포트")
INSURANCE = 104760 # 보험료 고정 (필요시 이것도 설정으로 뺄 수 있음)

# 설정된 시작일 기준으로 정산 기간 계산
