...
# [수정] 디자인 복구: 중앙 정렬(centered)로 돌리고 사이드바는 항상 보이게 설정
st.set_page_config(
    ...
    try:
        ...
        for col in columns:
            if col not in df.columns:
                df[col] = 0 if any(x in col for x in ["item", "인센티브", "합계"]) else ""
...
with st.sidebar:
    ...
    if st.button("로그아웃"):
        ...
        st.rerun()
    st.divider()
    st.caption(f"App Version: {SW_VERSION}")
...
# 실적 데이터 로드 및 수량 입력칸 로직 (v2.1.9와 동일)
